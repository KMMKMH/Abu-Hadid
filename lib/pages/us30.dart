...
import 'package:abu_hadid_app/pages/us30Signal.dart';

class US30 extends StatefulWidget {
  final bool admin;
  final Socket socket;
  const US30({
    super.key,
  ...
  @override
  State<US30> createState() => _US30State();
}

class _US30State extends State<US30> {
  ...
  Widget _buildSignalList() {
    return StreamBuilder(stream: _chatService.getSignalsFromUS30(), builder: (context, snapshot) {
...
${data['analysis']}
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(content: Row(
                                ...
                                children: [
                                  ...
                                  TextButton(
                                      onPressed: () {
                                        FirebaseFirestore.instance.collection('signal_room').doc('us30_room').collection('signals').doc(document.id).delete();
                                        ScaffoldMessenger.of(context).hideCurrentSnackBar();
                                      },
                                      child: Text(
                                        'Delete',
  ...
  Widget _buildSignalsAdder(bool admin) {
    return FloatingActionButton(
      onPressed: () {
        if (admin) {
          Navigator.push(context, MaterialPageRoute(builder: (context) => US30Signal(
