...
import 'package:abu_hadid_app/pages/oilSignal.dart';
...
class _OilState extends State<Oil> {
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          _buildSignalsAdder(widget.admin),SizedBox(
            height: 5,
          )
  ...
  Widget _buildSignalList() {
    return StreamBuilder(stream: _chatService.getSignalsFromOil(), builder: (context, snapshot) {
...
${data['analysis']}
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(content: Row(
                                ...
                                children: [
                                  ...
                                  TextButton(
                                      onPressed: () {
                                        FirebaseFirestore.instance.collection('signal_room').doc('oil_room').collection('signals').doc(document.id).delete();
                                        ScaffoldMessenger.of(context).hideCurrentSnackBar();
                                      },
                                      child: Text(
                                        'Delete',
  ...
  Widget _buildSignalsAdder(bool admin) {
    return FloatingActionButton(
      onPressed: () {
        if (admin) {
          Navigator.push(context, MaterialPageRoute(builder: (context) => OilSignal(
